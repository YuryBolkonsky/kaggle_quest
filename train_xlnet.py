...
# for i in {0..4}; do python -m torch.distributed.launch --nproc_per_node=2 train_dist_mergeclass_aug.py --kfold_rank=$i;done
import argparse
import os
import pickle
import re
...
def main():
    ...
    # criterion = torch.jit.trace(criterion.forward,
    #                             (torch.rand(BATCH_SIZE, 30).cuda(), torch.rand(BATCH_SIZE, 30).cuda()))
    ...
    def compute_loss(questions, answers, labels, criterion):
        ...
        outputs = model(inputs, attention_mask=attention_mask, token_type_ids=None)[0]  
        loss = criterion(outputs, labels)
        return outputs, loss

    model = create_xlnet_model(PRETRAIN_MODEL)
    ...
    for kfold_it, (train_idx, valid_idx) in enumerate(kfold.split(df, groups=df.question_body)):
        ...
        train_dataset = BucketDataset(
            ...
            tokenizer, crop=True, max_len=MAX_LEN, emb_dropout=None)
        ...
        valid_dataset = BucketDataset(
            ...
            tokenizer, crop=False, max_len=1024-1, emb_dropout=None)

        def pad_collate_fn(x): return pad_collate(x, tokenizer.pad_token_id)
        ...
        if args.local_rank == 0:
            writer.add_scalar('kfold', score, kfold_it)
            writer.flush()
            torch.save(model.module.state_dict(),
